...
function updateVisitCount() {
	fetch('https://api.countapi.xyz/update/OmarHosam/Watermelon/?amount=1')
	.then(res => res.json())
	.then(res => {
...
copybutton.addEventListener('click', function() {
	...
	genLink.innerHTML = "Copied into the clipboard!";
});
		...
		if (valid = false){
			...
			if (!minute.match(/\S/) && !second.match(/\S/) && !hour.match(/\S/)) 
    		{
		    	...
		    	if (url.includes("https://www.youtube.com/watch?v=")){
		    		...
		    		genLink.innerHTML = "Link has been generated successfully, Click on the Copy Button to Copy it.";
